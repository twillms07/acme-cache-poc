...
class CacheActorManagerSpec extends ScalaTestWithActorTestKit with WordSpecLike {
    ...
    "CacheManagerActor " must {
        "Create a Cache Actor if one doesn't exist for the given key " in {
            val testCacheManager: ActorRef[CacheActorManager.CacheActorManagerMessage] = spawn(CacheActorManager())
            ...
            testHttpServerProbe.expectMessage(max = 10 seconds, BackendValue("TestBackendResponse-testRequest1"))
        ...
        "Remove a Cache Actor if one exists for a given key " in {
            ...
            testHttpServerProbe.expectMessage(max = 10 seconds, BackendValue("TestBackendResponse-testRequest2"))
            testCacheManager ! GetCacheList(testHttpServerProbe.ref)
